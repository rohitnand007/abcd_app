class QuizzesController < ApplicationController
  ...
  def index
    ...
    if @filter_center.to_i == 0
      @quizzes = Quiz.where("id > ?",0).order('id DESC').page(params[:page])
    elsif @filter_center == 'IA'
      @quizzes = Quiz.where("id > ? AND (institution_id=? OR center_id (IN) ?)",0,current_user.institution_id,current_user.centers.map(&:id)).order('id DESC').page(params[:page])
    else
      @quizzes = Quiz.where("id > ? AND center_id = ?",0,@filter_center).order('id DESC').page(params[:page])
    end
  ...
  def all_other_assessments
    ...
    @centers = get_centers
    @quizzes = Quiz.where("institution_id != ?",EDUTOR).order('id DESC').page(params[:page])
  ...
  def my_institutes_assessments
    @quizzes = Quiz.where("institution_id = ?",current_user.institution_id).order('id DESC').page(params[:page])
        ...
        @builder = Nokogiri::XML::Builder.new(:encoding => 'UTF-8') do |xml|
          xml.testpaper(:id=>@quiz.id.to_s,:version=>"2.0",:publish_id=>@publish.id.to_s){
            ...
            @quiz.questions.each do |question|
              ...
              xml.question_set(:id=>question.id){
                xml.section_number{
...
=end
  ...
  def create_uri(context)
    if (!@publish.extras.nil? and @publish.extras.index("homework"))
      if context.topic_id !=0
        uri = "/Curriculum/HomeWork/#{context.topic.board.name}/#{context.topic.content_year.name}/#{context.topic.subject.name}/#{context.topic.chapter.name}/#{context.topic.name}/#{@quiz.name}"
      elsif context.chapter_id !=0# AssessmentInTopicQuiz For Chapter
        uri = "/Curriculum/HomeWork/#{context.chapter.board.name}/#{context.chapter.content_year.name}/#{context.chapter.subject.name}/#{context.chapter.name}/#{@quiz.name}"
      elsif context.subject_id !=0# AssessmentInTopicQuiz For Subject
        uri = "/Curriculum/HomeWork/#{context.subject.board.name}/#{context.subject.content_year.name}/#{context.subject.name}/#{@quiz.name}"
      end
    else
      if context.topic_id !=0
        uri = "/Curriculum/Assessment/#{context.topic.board.name}/#{context.topic.content_year.name}/#{@publish.get_assessment_ncx}/#{context.topic.subject.name}/#{context.topic.chapter.name}/#{context.topic.name}/#{@quiz.name}"
      elsif context.chapter_id !=0# AssessmentInTopicQuiz For Chapter
        uri = "/Curriculum/Assessment/#{context.chapter.board.name}/#{context.chapter.content_year.name}/#{@publish.get_assessment_ncx}/#{context.chapter.subject.name}/#{context.chapter.name}/#{@quiz.name}"
      elsif context.subject_id !=0# AssessmentInTopicQuiz For Subject
        uri = "/Curriculum/Assessment/#{context.subject.board.name}/#{context.subject.content_year.name}/#{@publish.get_assessment_ncx}/#{context.subject.name}/#{@quiz.name}"
      end
    end
  end
  ...
  def myquizzes
    ...
    @quizzes = Quiz.where("createdby = ? AND id IN (SELECT quiz_id FROM `quiz_targeted_groups`)",current_user.id).order('timecreated DESC').page(params[:page])
  ...
  def myunpublishedquizzes
    ...
    @quizzes = Quiz.where("createdby = ? AND id NOT IN (SELECT quiz_id FROM `quiz_targeted_groups`)",current_user.id).order('timecreated DESC').page(params[:page])
...
"student_info":[
  ...
  def show_institute_tests
    @quizzes = Quiz.where(:institution_id=>current_user.institution_id).order("id DESC")
  end
    ...
    if params[:export_type] == "Export as CSV"
      ...
      if @quiz.format_type == 0
        attempts.each do |a|
          ...
          user = User.find(a.id)
          all_attempts << [a.edutorid,user.school_uid,a.name,(user.academic_class.name rescue ""),(user.section.name rescue ""),i,a.marks,Time.at(a.time).to_datetime.strftime("%d-%b-%Y %I:%M %p")]
          i = i+1
        end
      else
        attempts.each do |a|
          user = User.find(a.id)
          all_attempts << [a.edutorid,user.school_uid,a.name,(user.academic_class.name rescue ""),(user.section.name rescue ""),a.tag,a.marks,Time.at(a.time).to_datetime.strftime("%d-%b-%Y %I:%M %p")]
        end
      end
      ...
      csv_data = FasterCSV.generate do |csv|
        ...
        csv << ['Published on:',display_date_time(@details.published_on)]
        csv << [" "]
        csv << "Edutorid,Admission,Student Name,Class,Section,Question,Marks,Submit Time".split(",")
        all_attempts.each do |c|
          csv << c
        end
      end
      ...
      @attempts = []
      #if @quiz.format_type == 0
      attempts.each_slice(questions_count).each do |a|
        marks =  a.collect{|i|  i.marks.to_f}
        logger.info "==========#{marks}"
        user = User.includes(:academic_class,:section).find(a.first.id)
        @attempts << [user.edutorid,user.school_uid,user.name,(user.academic_class.name rescue ""),(user.section.name rescue "")]+marks+[marks.sum.to_i,Time.at(a.last.time).to_datetime.strftime("%d-%b-%Y %I:%M %p")]
      end
  ...
  def start_assessment
    ...
    if !@publish.nil?
      ...
      if @quiz.format_type == 1
        respond_to do |format|
          format.html { render "take_openformat_assessment2" }
          format.json { render json: @quiz.errors, status: :unprocessable_entity }
        end
      else
  ...
  def show_student_attempts
    @publish = QuizTargetedGroup.find(params[:id])
    @quiz = Quiz.find(@publish.quiz_id)
    @attempts = QuizAttempt.where(:quiz_id=>@quiz.id,:publish_id=>@publish.id,:user_id=>current_user.id).order("id DESC")
    @total_marks = @quiz.quiz_question_instances.sum(:grade)
    if (@quiz.accessible(current_user.id, current_user.institution_id))
      respond_to do |format|
        format.html
          ...
          ActiveRecord::Base.transaction do
            ...
            Dir[directory+"/"+'/*.etx'].each do |etxfile|
              ...
              @etx.xpath("//question_set").each do |q|
               ...
               # question.defaultmark = q.xpath("score").attr("value").to_s.to_f
                question.penalty = q.xpath("negativescore").attr("value").to_s
...
#    @results = TestResult.where(:uid=>@quiz.id,:publish_id=>@publish.id).group(:user_id)
   ...
   @publish = QuizTargetedGroup.find(params[:id])
    ...
    @question_ids = @quiz.questions.map(&:id)
    @question_texts = @quiz.questions.order('id').collect{|q|q.questiontext.gsub(/<\/?[^>]*>/, "").gsub(',',"")}
    ...
    attempts.each do |a|
      ...
      @attempts << [user.name,user.edutorid]+ans
    end

    csv_data = FasterCSV.generate do |csv|
      csv << "User,Edutorid,#{@question_ids.join(',')}".split(',')
  ...
  def get_quiz_message
   ...
   def practice_assessments
    if current_user.institution_id
      @quizzes =  Quiz.includes(:quiz_targeted_groups,:quiz_attempts=>[:user]).where("users.institution_id=#{current_user.institution_id} AND quiz_targeted_groups.extras='practice'").page(params[:page])
    end
  end
