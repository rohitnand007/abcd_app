class SsoProvider < ActiveRecord::Base
  ...
  def authenticate(redirect_uri, code, nonce)
    begin
      ...
      result = {:access_token=>access_token.access_token, :id_token => access_token.id_token,:status=>true}
    rescue
      result = {:status=>false}
    end
    return result
  ...
  class << self
    def discover!(host)
      ...
      if sso_provider = find_by_issuer(issuer)
        sso_provider
      else
        dynamic.create(
            issuer: issuer,
