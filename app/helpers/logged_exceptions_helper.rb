module LoggedExceptionsHelper
  def pretty_exception_date(exception)
    if Date.today == Time.at(exception.created_at).to_date
      if Time.at(exception.created_at) > Time.now - 4.hours
        "#{time_ago_in_words(Time.at(exception.created_at)).gsub(/about /,"~ ")} ago"
      else
        "Today, #{Time.at(exception.created_at).strftime(Time::DATE_FORMATS[:exc_time])}"
      end
    else
      Time.at(exception.created_at).strftime(Time::DATE_FORMATS[:exc_date])
    end
  end
